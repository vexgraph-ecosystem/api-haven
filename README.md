...
* **`api/haven_ws_fanout.h/.c`** — L2 fan-out registry (`HavenWsFanout`): 16-slot registry over opaque transport handles + `HavenWsSource` fn-table `{connect, poll, send, close}`; `pollStep` budget-driven by R1 with even millisecond slices, zero threads, zero socket syscalls.
...
* **`tests/api-haven/haven_ws_fanout_test.c`** — Headless proof: builds `haven_ws_fanout_test` verifying 16-slot registry, duplicate handling, capacity bounds, budget-slicing poll, and detaching.
